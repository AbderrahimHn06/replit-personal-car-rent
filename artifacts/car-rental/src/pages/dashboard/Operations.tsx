  ...
  if (tab === "rentals" || tab === "offline") {
    ...
    rentalsData.forEach((r, i) => {
      ...
      cells.forEach((cell, ci) => {
        doc.text(doc.splitTextToSize(cell, colWidths[ci] - 2)[0], x, y);
        x += colWidths[ci];
      });
    ...
    if (rentalsData.length === 0) {
      ...
      doc.setFontSize(10);
      doc.text("No records found.", pw / 2, y + 10, { align: "center" });
    }
  }
...
export function OperationsSection({ activeTab, onTabChange }: Props) {
  ...
  function applyBookingSearch(list: BookingRequest[]): BookingRequest[] {
    return list.filter(r =>
      !search || [r.customer, r.car, r.email, r.phone].some(v => v.toLowerCase().includes(search.toLowerCase()))
    );
  }

  function handleExportPDF() {
    if (activeTab === "bookings") {
      generateRentalsPDF("bookings", [], applyBookingSearch(bookingRequests), search);
    } else if (activeTab === "offline") {
      generateRentalsPDF("offline", applyRentalSearch(allRentals.filter(r => r.source === "walk-in")), [], search);
    } else {
      generateRentalsPDF("rentals", applyRentalSearch(allRentals), [], search);
    }
  }
