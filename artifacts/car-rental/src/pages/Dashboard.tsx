...
const LABELS: Record<Section, { title: string; sub: string }> = {
  ...
  bookings: { title: "Booking Requests", sub: "Online requests from the website" },
  offline: { title: "Walk-in Rentals", sub: "Counter and office rentals" },
  rentals: { title: "Rentals", sub: "All active and historical rentals" },
...
export function Dashboard({ bookings }: { bookings: Booking[] }) {
  ...
  return (
    <div className="flex h-screen bg-[#F5F6F8] overflow-hidden">
      ...
      >
        {/* Logo */}
        <div className="flex items-center justify-between h-[60px] px-5 border-b border-slate-100">
        ...
        <div className="border-t border-slate-100 p-3 space-y-1">
          <Link href="/">
            <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-[13px] text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors">
          ...
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-semibold text-slate-700 truncate">Agency Manager</p>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="h-[60px] bg-white border-b border-slate-200/80 flex items-center justify-between px-5 flex-shrink-0 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-sm font-bold text-[#1a2332] leading-tight">{title}</h1>
              <p className="text-[11px] text-slate-400 leading-tight hidden sm:block">{sub}</p>
            </div>
          </div>
